...
    
```
- 第一个if判断语句检查一个人是否未满4岁。如果是，Python就打印一条合法免费进入游乐场的消臭。并跳过余下的所有判断测试。第二个elif代码其实是另一个if测试，它仅在前面的测试未通过时才会运行。在这里，我们知道这个人小于4岁，以为第一个条件测试未通过。如果这个人满18岁，Python将打印相应的消息，