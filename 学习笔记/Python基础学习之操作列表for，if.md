...
- ![](assets/Python基础学习之操作列表for，if/file-20260112013413705.png)
...
- ![](assets/Python基础学习之操作列表for，if/file-20260112013413705%201.png)
...
- ![](assets/Python基础学习之操作列表for，if/file-20260112013413706.png)
...
- ![](assets/Python基础学习之操作列表for，if/file-20260112013413705%202.png)
...
- 下面是一个制作煎饼果子前检查顾客点的配料列表是否为空。如果列表为空，就向顾客确认是否要点原味的煎饼果子；如果列表非空，就按照配料表给顾客制作煎饼果子。
```python
jianbing_peiliao = []
if jianbing_peiliao:
    for jianbing_peiliao in jianbing_peiliao:
        print(f"在煎饼果子里添加{jianbing_peiliao}")
else:
    print("你没有添加任何配料,确定要原味的煎饼果子吗?")
    
```
- 首先创建一个空列表，其中不包含任何配料。然后进行简单的检查，而不是直接执行for循环。在if语句中将列表名用作条件表达式时，python将在列表至少包含一个元素时返回True，在列表为空时返回False。如果jianbing_peiliao非空，就运行与前一个示例相同的for循环；否则打印一条消息，询问顾客确实要点原味的煎饼果子吗？
- ![](assets/Python基础学习之操作列表for，if/file-20260112013413706%201.png)
## 使用多个列表
- 顾客的要求五花八门，在做煎饼果子配料方面尤其如此。如果顾客要再煎饼果子中添加“火腿肠”，改怎么办呢？可以使用列表和if语句来确定能否满足顾客的要求。
- 我们来看看如何在制作煎饼果子时拒绝怪异的配料要求，下面的示例定义了两个列表，其中第一个列表是包含我们做煎饼果子的所有配料表，第二个列表是顾客提交的配料比表。这次对顾客提交的user_add每一个配料都检查是否是在我们供应的煎饼果子的配料中，在决定是否在做煎饼果子时添加它。
```python
jianbing_peiliao = ['火腿','鸡蛋','脆饼','大葱','油条','五花肉']
user_add = ['大蒜','火腿','大葱','油条','辣椒']

for user_add in user_add:
    if user_add in jianbing_peiliao:
        print(f"做煎饼果子可以添加:{user_add}")
    else:
        print(f"不好意思,我们没有 {user_add} 这个配料")
        
```
代码解说：首先定义一个煎饼果子的配料表，其中包含做煎饼果子时可以添加的所有配料，如果煎饼果子的配料表是固定的也可以用一个元组来存储它们。然后创建另一个列表user_add用来存储顾客要添加的配料。接下来遍历用户的配料列表，在这个循环中，对于顾客点的每种配料，都检查它是否在我们提供的配料表里。如果的确是在我们提供的配料表里，就将其加煎饼果子里，否则运行else代码块：打印一条消息，告诉顾客我们没有这种配料。
这些是代码输出的结果：
- ![](assets/Python基础学习之操作列表for，if/file-20260113162200320.png)
## 设置if语句的格式
...
```
更好。
这样的空格不会影响python对代码的解读，这是让代码读起来更容易。
